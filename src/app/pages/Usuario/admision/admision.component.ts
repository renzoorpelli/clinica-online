...
export class AdmisionComponent implements OnInit, OnDestroy{
  ...
  allowUser(item:Usuario){
    ...
    if(item.rol === "especialista"){
      this._especialistaRepositoryService.update(item.idUsuarioDocRef!, "admitido");

    }else{
      this._pacienteRepositoryService.update(item.idUsuarioDocRef!, "admitido");
    }
  ...
  denyAccess(item:Usuario){
    ...
    if(item.rol === "especialista"){
      ...
      //this._especialistaRepositoryService.delete(docRefEspecialista);
